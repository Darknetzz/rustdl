...
- **Rate control**: Bitrate by default (empty target → ~2 Mbps auto), or Quality (**CRF**, typically 30). Lower CRF is higher quality and larger files. CPU encoders use ffmpeg `-crf`; NVIDIA uses `-cq`; AMD uses QP. The size preset still scales bitrate or offsets CRF.
- **Cap bitrate to source** (bitrate mode, off by default): never sets `-b:v` / `-maxrate` above the probed source bitrate, so low-bitrate clips are not re-encoded larger by the 2 Mbps auto fallback.
- **Size limit** (default **Min shrink 30%**): skips (or fails, depending on violation mode) encodes that would not shrink enough vs the source. Set to Off under Settings → Converter if you want to keep larger outputs.
