...
- Video Converter now accepts **FLV** inputs (scan, file picker, and watch folder), same as other ffmpeg-readable containers.
- Video Converter no longer throws away a finished encode when **Delete original** fails because the source is in use (Windows). The converted file is kept, hardware→CPU retry is skipped, and Retry only finishes delete/rename if the output already looks complete. Convert also fails fast before encoding when the source is already locked.
...
- Settings **General** tab (formerly Shared) is split into sub-sections: **Appearance**, **Panels & log**, **System**, **Tools**, and **Backup** (with icons); the last-opened sub-section is remembered.
...
- **More info** file metadata no longer re-runs ffprobe on every open for audio-only downloads (or when optional container tags are missing); probed fields are saved to the queue file again.
